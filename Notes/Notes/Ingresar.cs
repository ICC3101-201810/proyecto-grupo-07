...
namespace Notes
{
    public class Ingresar
    {
        ...
        { //Rut
            ...
            char[] auxMiRut = miRut.Trim().ToCharArray();
            if (auxMiRut.Last() == 'k')
            {
            ...
            {   //Vemos que sea un numero rasonable
                if ((7 < miRut.Trim().Count()) && (9 > miRut.Trim().Count()))
                {
                    return new int[] { int.Parse(miRut.Trim()) };
                }
            }
            //Si se ingreso solo con '-'
            else if (miRut.Trim().Split('-').Count() == 2)
                    ...
                    {  //Que sea un valor rasonable
                             ...
                             miRut.Trim().Split('-')[1].Trim().Count() == 1)
                        {
                            return new int[] {int.Parse(miRut.Trim().Split('-')[0].Trim()) +
                                                  int.Parse(miRut.Trim().Split('-')[1].Trim())};
                        }
                    }
                }     
            ...
            {   //Se podria haber simplificado muuuucho si simplemente sacaba los espacios blancos al
                //principio, no se me ocurrio antes :(, o se veria mas lindo con variables
